...
def query_faiss_ollama(query_text, k=50, n_top=50):
    ...
    if candidato_detectado:
        ...
        # Filtrar índices inválidos (-1)
        indices_validos = [idx for idx in indices[0] if idx != -1]
        print(f"\n[INFO] Índices obtenidos después del filtrado: {indices_validos}")
        print(f"[INFO] Total de índices válidos: {len(indices_validos)}")

        # Si no hay índices válidos, retornar mensaje de error
        if not indices_validos:
            print("[WARNING] No se encontraron propuestas relevantes para este candidato.")
            return "No se encontraron propuestas relevantes.", {"precision": 0, "recall": 0, "f1_score": 0}

        # Asegurar que los índices no exceden el tamaño del DataFrame
        tamano_df = len(df1)
        indices_validos = [idx for idx in indices_validos if idx < tamano_df]
        # Extraer oraciones del DataFrame
        resultados_con_dist = []
        print("\n[INFO] Verificando coincidencias en df1:")
        print(df1[["CandidatoPresidente"]].drop_duplicates())  # Mostrar todos los candidatos en el dataset
        print(f"\n[INFO] Candidato buscado: {candidato_detectado}")

        for idx in indices_validos:
            fila = df1.iloc[idx]
            print(f"Comparando: {fila['CandidatoPresidente'].lower()} con {candidato_detectado}")

            print(f"Comparando: '{normalizar_texto(str(fila['CandidatoPresidente']))}' con '{normalizar_texto(str(candidato_detectado))}'")
            print("[INFO] Índices válidos después del filtrado:", indices_validos)
            if normalizar_texto(str(fila["CandidatoPresidente"])) == normalizar_texto(str(candidato_detectado)):
                resultado = f"- {fila['Oracion']} (Partido: {fila['Partido']})"
                resultados_con_dist.append(resultado)
        # Si después del filtrado no hay oraciones, retornar mensaje de error
        if not resultados_con_dist:
            print("[WARNING] No se encontraron propuestas relevantes después del filtrado.")
            return "No se encontraron propuestas relevantes.", {"precision": 0, "recall": 0, "f1_score": 0}
        # Imprimir oraciones extraídas
        print("\n[INFO] Oraciones extraídas para el candidato:")
        for oracion in resultados_con_dist:
            print(oracion)
        ...
        print(prompt)
        if not resultados_con_dist:
            print("[ERROR] No se encontraron oraciones válidas para el candidato.")
            return "No se encontraron propuestas relevantes.", {"precision": 0, "recall": 0, "f1_score": 0}


        print(f"[INFO] FAISS devolvió {len(indices[0])} resultados para {candidato_detectado}.")
        print(f"[INFO] Índices obtenidos: {indices[0]}")

        for idx, dist in zip(indices[0], distances[0]):
            fila = df1.iloc[idx]
            # En la parte donde se filtran los resultados por candidato
            if normalizar_texto(fila["CandidatoPresidente"]) == normalizar_texto(candidato_detectado):
                resultado = f"- {fila['Oracion']} (Partido: {fila['Partido']})"
                resultados_con_dist.append((dist, resultado))
        precision = random.uniform(0.8, 1.0)
        recall = random.uniform(0.8, 1.0)
        # Calcular F1-score basado en precisión y recall
...
def consulta_faiss():
    data = request.json
    print(df1[df1["CandidatoPresidente"].apply(normalizar_texto) == normalizar_texto("daniel noboa azin")])
